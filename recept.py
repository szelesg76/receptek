...
async def uzletek(request: Request):
...
async def uzlet_letrehozas_mentes(request: Request, uzlet_nev: str = Form(...)):
    """Új üzlet rögzítése
...
async def uzlet_modosit(request: Request, uzlet_id: int):
    
    uzlet = UZLETEK[uzlet_id-1]
    
    return templates.TemplateResponse("uzlet_modosit.html", {"request": request, "uzlet": uzlet})



@app.post("/uzlet_modosit/{uzlet_id}", response_class=HTMLResponse)
async def uzlet_modosit_mentes(request: Request, uzlet_id: int, uzlet_nev: str = Form(...)):
    """Új üzlet rögzítése
...
async def uzlet_torol(request: Request, uzlet_id: int):
